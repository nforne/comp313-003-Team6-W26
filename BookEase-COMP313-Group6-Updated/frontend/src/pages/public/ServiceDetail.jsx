...
import { ArrowLeft, MapPin, Clock, DollarSign, User, Star, Send, Check } from 'lucide-react';
...
const ServiceDetail = () => {
  ...
  const handleAcceptBid = async (bidId) => {
    try {
      await bookingsAPI.create({ serviceId: id, bidId });
  ...
  const isOwner = user && service.customer?._id === user.id;
  const isProvider = user?.role === 'provider';
  const hasMyBid = service.bids?.some((b) => b.provider?._id === user?.id);
            ...
            <div className="space-y-3">
              {service.bids?.map((bid) => (
                ...
                >
                  ...
                  {bid.status && (
                    <span className={`apple-badge text-[11px] mt-2 ml-[52px] status-${bid.status}`}>
                      {bid.status}
                    </span>
                  )}
